...
const ItemList = () => {
  const [tableData, setTableData] = useState(ITEM_TABLE_DATA);
  ...
  const handleView = (record) => console.log("View item:", record);

  // Edit now opens AddItemModal, prefilled with the selected row
  const handleEdit = (record) => {
    setEditingItem(record);
    setIsAddModalOpen(true);
  };

  const handleDelete = (record) => console.log("Delete item:", record);
  ...
  <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl  p-3">
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      <FilterDropdown
        label="Item Name"
        value={itemFilter}
        options={ITEM_NAME_FILTER_OPTIONS}
