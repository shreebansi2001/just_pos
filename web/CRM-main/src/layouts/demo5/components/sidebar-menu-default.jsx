...
import { filterMenuByPermission } from '@/auth/lib/normalizeRights';
  ...
  const { user } = useAuth();
  const roleId = Number(user?.roleId || user?.userBasicDetails?.role?.id || 0);
  const isSuperUser = roleId === 1 || roleId === 2;

  const rawItems = getChildren(MENU_SIDEBAR, 0);
...
const menuItems = filterMenuByPermission(rawItems || [], rights, isSuperUser);
